import { Database, Lock, Mail, ShieldCheck } from "lucide-react";

const privacySections = [
  {
    title: "مصادر الزيارات والتحليلات",
    text: "بعد موافقتك على ملفات التحليلات والإعلانات، نستخدم Google Analytics وGoogle Ads وMeta Pixel لقياس الزيارات والتسجيلات الناجحة. قد تتلقى هذه الخدمات عنوان الصفحة ومصدر الزيارة ومعرّفات ملفات الارتباط ومعلومات المتصفح والاتصال. نرسل إلى Meta حدث إكمال التسجيل بعد تأكيد البريد وتجهيز المتجر، مع معرّف غير سري لمنع التكرار. نحفظ أول مصدر معروف وآخر حملة معروفة على جهازك لمدة تصل إلى 90 يومًا، وقد نربطهما بحساب متجرك عند التسجيل. لا نرسل كلمة المرور أو رمز التحقق أو بيانات الاتصال في أحداث التسجيل، ولا نفعّل المطابقة المتقدمة لبيانات النماذج. يمكنك رفض القياس أو تغيير اختيارك من إعدادات ملفات الارتباط. سؤال «عرفتنا منين؟» اختياري وتُحفظ إجابته منفصلة عن التتبّع التلقائي.",
  },
  {
    title: "المعلومات التي نجمعها",
    text: "عند إنشاء حساب أو التواصل معنا، قد نجمع بيانات مثل الاسم، البريد الإلكتروني، رقم الهاتف، اسم المتجر، وبيانات الاستخدام الأساسية داخل المنصة.",
  },
  {
    title: "كيف نستخدم البيانات",
    text: "نستخدم البيانات لتشغيل حسابك، تحسين تجربة المتجر، تقديم الدعم، إرسال تنبيهات مهمة، ومساعدتك في إدارة المنتجات والطلبات.",
  },
  {
    title: "مشاركة البيانات",
    text: "لا نبيع بياناتك الشخصية. قد نشارك بعض المعلومات فقط عند الحاجة لتشغيل الخدمة، مثل خدمات الاستضافة أو الدعم الفني.",
  },
  {
    title: "حماية المعلومات",
    text: "نستخدم إجراءات تنظيمية وتقنية لحماية بياناتك من الوصول غير المصرح به، ونعمل على تقليل الوصول للبيانات حسب الحاجة.",
  },
];

export function PrivacyPage() {
  return (
    <div className="min-h-screen bg-white py-16 text-right" dir="rtl">
      <div className="container mx-auto px-4 md:px-6">
        <section className="hero-band noise-grid mx-auto mb-12 max-w-4xl rounded-lg border border-emerald-100 p-8 text-center md:p-10">
          <div className="icon-tile mx-auto mb-5 h-14 w-14">
            <Lock className="h-7 w-7" />
          </div>
          <h1 className="mb-4 text-3xl font-black text-gray-950 font-heading md:text-4xl">
            سياسة الخصوصية
          </h1>
          <p className="mx-auto max-w-2xl text-lg leading-relaxed text-gray-600">
            نوضح هنا كيف تتعامل متجركو مع بياناتك وبيانات متجرك، وما الذي
            نستخدمه لتقديم الخدمة وتحسينها.
          </p>
        </section>

        <section className="mx-auto grid max-w-5xl gap-6 md:grid-cols-2">
          {privacySections.map((section) => (
            <article
              key={section.title}
              className="premium-card p-7"
            >
              <h2 className="mb-3 text-lg font-black text-gray-950 font-heading">
                {section.title}
              </h2>
              <p className="leading-relaxed text-gray-600">{section.text}</p>
            </article>
          ))}
        </section>

        <section className="mx-auto mt-10 max-w-5xl rounded-lg border border-gray-200 bg-gray-50 p-8">
          <div className="grid gap-6 md:grid-cols-3">
            <div className="flex items-start gap-3">
              <ShieldCheck className="mt-1 h-5 w-5 shrink-0 text-emerald-700" />
              <p className="text-gray-700">نحافظ على سرية بيانات حسابك ومتجرك.</p>
            </div>
            <div className="flex items-start gap-3">
              <Database className="mt-1 h-5 w-5 shrink-0 text-emerald-700" />
              <p className="text-gray-700">تستخدم البيانات لتشغيل الخدمة وتحسينها.</p>
            </div>
            <div className="flex items-start gap-3">
              <Mail className="mt-1 h-5 w-5 shrink-0 text-emerald-700" />
              <p className="text-gray-700">للاستفسار: matgarko.help@gmail.com</p>
            </div>
          </div>
        </section>
      </div>
    </div>
  );
}
