...
Tracking requires consent and `VITE_GA_MEASUREMENT_ID`. Page visits, signup steps, and the completed `sign_up` event are supported. Optional direct Google Ads tracking also requires `VITE_GOOGLE_ADS_ID` and `VITE_GOOGLE_ADS_SIGNUP_LABEL`. Values are public build-time configuration; rebuild after changes. Do not add a second static Google tag to `index.html`.
